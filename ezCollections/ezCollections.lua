...
for k, v in pairs({
    ...
    RaceNameToFaction = { HUMAN = FACTION_ALLIANCE, DWARF = FACTION_ALLIANCE, NIGHTELF = FACTION_ALLIANCE, GNOME = FACTION_ALLIANCE, DRAENEI = FACTION_ALLIANCE, FELORC = FACTION_ALLIANCE, BROKEN = FACTION_ALLIANCE, WORGEN = FACTION_ALLIANCE, TAUNKA = FACTION_ALLIANCE, NORTHRENDSKELETON = FACTION_ALLIANCE, ORC = FACTION_HORDE, UNDEAD = FACTION_HORDE, TAUREN = FACTION_HORDE, TROLL = FACTION_HORDE, BLOODELF = FACTION_HORDE, NAGA_ = FACTION_HORDE, FORESTTROLL = FACTION_HORDE, SKELETON = FACTION_HORDE, ICETROLL = FACTION_HORDE, ANY = FACTION_OTHER },
    RaceSortOrder = { "HUMAN", "DWARF", "NIGHTELF", "GNOME", "DRAENEI", "FELORC", "BROKEN", "WORGEN", "TAUNKA", "NORTHRENDSKELETON", "ORC", "UNDEAD", "TAUREN", "TROLL", "BLOODELF", "NAGA_", "FORESTTROLL", "SKELETON", "ICETROLL" },
...
local function OnItemEquipped(event, player, item, bag, slot)
    local id = item:GetEntry()
    if player:GetName() == UnitName("player") then  -- Check if the equipped item is on the player
        if ezCollections.Config.TooltipCollection.SkinUnlock and not ezCollections:HasSkin(id) then
            local tooltip = CreateFrame("GameTooltip", "ItemTooltip", nil, "GameTooltipTemplate")
            ...
            if isCollectible then
                ...
                print("Unlocking skin for item ID:", id)
                ezCollections:UnlockSkinUnderCursor()
				ReloadAddon("ezCollections")
            end
            tooltip:Hide()
        end
    end
...
frame:RegisterEvent("PLAYER_EQUIP_ITEM")  -- Assuming this is the event for item equip
frame:SetScript("OnEvent", OnItemEquipped)